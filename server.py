    ...
    if 'Username' in session:
        ...
        if request.method == 'GET':
            try:
                ...
                return render_template('buy_ticket.html', flightid=flight_id, balance=session['Balance'],
                                       emptyseatsforeco=emptyseatsforeco, emptyseatsforbsn=emptyseatsforbsn,
                                       priceforeco=priceforeco, priceforbsn=priceforbsn)
            except dbapi2.DatabaseError:
                connection.rollback()
                return "Hata!"
            finally:
